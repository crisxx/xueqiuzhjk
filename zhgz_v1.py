...
def gpflash( str_url,zh_name,n):
	...
	stItems = re.findall(r'.*?<span class="stock-name"><div class="name">(.*?)</div>.*?<span class="stock-weight weight">(.*?)</span>', unicodePage)
	...
	print '    股票','   ','仓位'
	for st in stItems:
		print '   ', st[0].encode('utf-8'),'   ',st[1].encode('utf-8')
	print'|'
